...
DialogImportAuxData::DialogImportAuxData(QWidget* parent) : QDialog(parent), ui(new Ui::DialogImportAuxData) {
  ui->setupUi(this);
}
...
void DialogImportAuxData::on_btn_import_clicked() {
  if (ui->rb_file->isChecked()) {
    ...
    if (!info.exists() || !QFile::copy(info.absoluteFilePath(),
                                       QString::fromStdString(m_parent_folder) + QDir::separator() + info.fileName())) {
    ...
    if (ui->cb_createSubGroup->isChecked()) {
      ...
      if (!QDir().mkdir(dest_folder)) {
        QMessageBox::warning(this, "Folder Not Created...",
                             "It is not possible to create a group with the selected folder name.", QMessageBox::Ok);
        return;
      }
    }
