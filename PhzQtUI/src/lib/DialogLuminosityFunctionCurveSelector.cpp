...
DialogLuminosityFunctionCurveSelector::DialogLuminosityFunctionCurveSelector(DatasetRepo luminosity_repository,
                                                                             QWidget*    parent)
    : QDialog(parent), ui(new Ui::DialogLuminosityFunctionCurveSelector) {
  ui->setupUi(this);
...
void DialogLuminosityFunctionCurveSelector::on_btn_save_clicked() {
  ...
  if (curve.size() == 0) {
    QMessageBox::warning(this, "Missing Data...", "Please select a curve.", QMessageBox::Ok);
    return;
  }
