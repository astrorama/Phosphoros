...
bool FileUtils::copyRecursively(const QString& srcFilePath, const QString& tgtFilePath) {
...
std::string FileUtils::getRootPath(bool with_separator) {
  QString path = QString::fromStdString(getRootPaths()
                                            .getConfiguration<PhzConfiguration::PhosphorosRootDirConfig>()
                                            .getPhosphorosRootDir()
                                            .generic_string());
  if (with_separator) {
...
std::string FileUtils::getGUILuminosityPriorConfig(bool check, const std::string& catalog_type,
                                                   const std::string& model) {
  QString path = QString::fromStdString(FileUtils::getGUIConfigPath()) + QDir::separator() + "LuminosityPrior" +
                 QDir::separator() + QString::fromStdString(catalog_type) + QDir::separator() +
...
std::string FileUtils::getResultRootPath(bool check, const std::string& catalog_type,
                                         const std::string& cat_file_name) {
...
std::map<std::string, std::string> removeDefault(const std::string& root_path, const std::string& default_path,
                                                 const std::string& key, std::map<std::string, std::string> path_list) {
...
void FileUtils::savePath(const std::map<std::string, std::string>& path_list) {
  ...
  std::string default_path =
      getRootPaths().getConfiguration<PhzConfiguration::CatalogDirConfig>().getCatalogDir().generic_string();
  ...
  default_path =
      getRootPaths().getConfiguration<PhzConfiguration::IntermediateDirConfig>().getIntermediateDir().generic_string();
...
std::map<std::string, std::string> FileUtils::readPath() {
  ...
  if (map.find("AuxiliaryData") == map.end()) {
    map.insert(std::make_pair(
        "AuxiliaryData",
        getRootPaths().getConfiguration<PhzConfiguration::AuxDataDirConfig>().getAuxDataDir().generic_string()));
  }

  if (map.find("IntermediateProducts") == map.end()) {
    map.insert(std::make_pair("IntermediateProducts", getRootPaths()
                                                          .getConfiguration<PhzConfiguration::IntermediateDirConfig>()
                                                          .getIntermediateDir()
                                                          .generic_string()));
  }
  ...
  if (map.find("Results") == map.end()) {
    map.insert(std::make_pair(
        "Results",
        getRootPaths().getConfiguration<PhzConfiguration::ResultsDirConfig>().getResultsDir().generic_string()));
  }
...
std::string FileUtils::getPhotmetricGridRootPath(bool check, const std::string& catalog_type) {
  if (catalog_type.length() > 0) {
    QString path = QString::fromStdString(FileUtils::getIntermediaryProductRootPath(false, catalog_type)) +
                   QDir::separator() + "ModelGrids";
    QFileInfo info(path);
    if (check) {
      if (!info.exists()) {
        QDir().mkpath(path);
...
std::string FileUtils::getGalacticCorrectionGridRootPath(bool check, const std::string& catalog_type) {
  if (catalog_type.length() > 0) {
    QString path = QString::fromStdString(FileUtils::getIntermediaryProductRootPath(false, catalog_type)) +
                   QDir::separator() + "GalacticCorrectionCoefficientGrids";
    QFileInfo info(path);
    if (check) {
      if (!info.exists()) {
        QDir().mkpath(path);
...
std::string FileUtils::getFilterShiftGridRootPath(bool check, const std::string& catalog_type) {
  if (catalog_type.length() > 0) {
    QString path = QString::fromStdString(FileUtils::getIntermediaryProductRootPath(false, catalog_type)) +
                   QDir::separator() + "FilterVariationCoefficientGrids";
    QFileInfo info(path);
    if (check) {
      if (!info.exists()) {
        QDir().mkpath(path);
...
FileUtils::getPathConfiguration(bool add_cat, bool add_aux, bool add_inter, bool add_res) {
