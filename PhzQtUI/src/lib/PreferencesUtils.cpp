  ...
  if (file.open(QIODevice::ReadOnly)) {
    while (!file.atEnd()) {
      ...
      if (line_split.length() > 2) {
        ...
        map[catalog].insert(std::make_pair(line_split[1].toStdString(), val.toStdString()));
      }
    }
  }
...
PhysicsUtils::CosmologicalParameters PreferencesUtils::getCosmologicalParameters() {
  ...
  return PhysicsUtils::CosmologicalParameters{std::stod(value_omega_m), std::stod(value_omega_l),
...
void PreferencesUtils::setCosmologicalParameters(const PhysicsUtils::CosmologicalParameters& parameters) {
  setUserPreference("_global_preferences_", "Cosmological-Parameter-Omega-Matter",
                    QString::number(parameters.getOmegaM(), 'g', 20).toStdString());

  setUserPreference("_global_preferences_", "Cosmological-Parameter-Omega-Lambda",
                    QString::number(parameters.getOmegaLambda(), 'g', 20).toStdString());

  setUserPreference("_global_preferences_", "Cosmological-Parameter-Hubble",
