...
DialogResid::DialogResid(QWidget *parent) :
        QDialog(parent),
        ui(new Ui::DialogResid) {
      ui->setupUi(this);
    }
...
void DialogResid::checkComputePossible() {
	...
	if (!boost::filesystem::exists(basepath/"run_config.config")) {
	  ...
	  ui->lbl_warning->show();
	  ui->lbl_warning->setText("The configuration file is not present in the folder: please run again the template fitting.");
	  return;
   ...
   if (std::find(columns.begin(),columns.end(),"SED-Index")==columns.end()){
	   ui->btn_compute->setEnabled(false);
	   ui->lbl_warning->show();
	   ui->lbl_warning->setText("The result file do not contains the best fitted model: please run again the template fitting.");
   }
}
...
void DialogResid::on_btn_compute_clicked(){
  ...
  s3 << QString::fromStdString("--result-dir") <<  QString::fromStdString(m_folder)
	 << QString::fromStdString("--intermediate-product-dir") << QString::fromStdString(FileUtils::getIntermediaryProductRootPath(false, ""))
