...
DialogLuminositySedGroup::DialogLuminositySedGroup(QWidget* parent)
    : QDialog(parent), ui(new Ui::DialogLuminositySedGroup) {
  ui->setupUi(this);
}
  ...
  for (size_t i = 0; i < m_groups.size(); ++i) {
    ...
    if (group_1.second.size() == 0) {
      QMessageBox::warning(this, "Empty group",
                           "The SED Group '" + QString::fromStdString(group_1.first) +
                               "' is empty.\n"
    ...
    if (group_1.first.find_first_of("\t ") != std::string::npos) {
      QMessageBox::warning(this, "Space in the Name", "Please do not use space in the group names.", QMessageBox::Ok,
                           QMessageBox::Ok);
      return;
    }
