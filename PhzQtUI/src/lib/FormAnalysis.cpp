...
void FormAnalysis::adjustGridsButtons(bool enabled) {
  ...
  if (!name_ok) {
    ...
    tool_tip  = "The grid has not yet be computed.";
...
void FormAnalysis::setComputeCorrectionEnable() {
  ...
  if (gal_corr_needed && (!grid_gal_corr_name_exists || !grid_gal_ok)) {
    tool_tip = "Please run the Galactic Correction grid computation before computing the photometric corrections.";
  }

  if (filter_shift_grid_needed && (!grid_filter_shift_name_exists || !grid_filter_shift_ok)) {
    tool_tip = "Please run the Filter Shift Coefficient grid computation before computing the photometric corrections.";
  }
...
void FormAnalysis::setRunAnnalysisEnable(bool enabled) {
  ...
  if (grid_name_exists) {
    ...
    ui->btn_confLuminosityPrior->setToolTip(
        "You need to Generate the Model Grid before to configure the Luminosity Prior");
  }
  ...
  ui->btn_GetConfigAnalysis->setEnabled(grid_name_ok && (!need_gal_correction || grid_gal_corr_name_ok) &&
                                        (!filter_shift_grid_needed || grid_filter_shift_ok) && correction_ok &&
  ...
  ui->btn_RunAnalysis->setEnabled(
      ...
      (!filter_shift_grid_needed || (grid_filter_shift_name_exists && grid_filter_shift_ok)) && correction_ok &&
      correction_exists && lum_prior_ok && lum_prior_compatible && nz_prior_ok && run_ok && enabled);
  ...
  if (!(grid_name_ok && (!need_gal_correction || grid_gal_corr_name_ok) && correction_ok && lum_prior_ok &&
        lum_prior_compatible && nz_prior_ok && run_ok)) {
  ...
  if (!(grid_name_exists && (!need_gal_correction || (grid_gal_corr_name_exists && grid_gal_ok)) && correction_ok &&
        correction_exists && lum_prior_ok && lum_prior_compatible && nz_prior_ok && run_ok)) {
...
std::map<std::string, boost::program_options::variable_value> FormAnalysis::getGridConfiguration() {
  ...
  auto config = PhzGridInfoHandler::GetConfigurationMap(
      ...
      ui->lbl_lum_filter->text().toStdString(), ui->cb_igm->currentText().toStdString());
