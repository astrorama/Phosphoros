...
LuminosityPriorConfig LuminosityPriorConfig::deserialize(QDomDocument& doc){
 ...
 std::map<std::string, boost::program_options::variable_value> LuminosityPriorConfig::getConfigOptions() const {
  ...
  for (auto& function : m_luminosity_function_list) {
    ...
    options["luminosity-function-sed-group-" + name].value() = boost::any(
        function.sedGroupName);
    options["luminosity-function-min-z-" + name].value() = boost::any(
        function.z_min);
    options["luminosity-function-max-z-" + name].value() = boost::any(
        function.z_max);

    if (function.is_custom) {

      options["luminosity-function-curve-" + name].value() = boost::any(
          function.curve_name);
    } else {
      options["luminosity-function-schechter-alpha-" + name].value() =
          boost::any(function.alpha);

      if (m_in_mag) {
        options["luminosity-function-schechter-m0-" + name].value() =
            boost::any(function.m);
      } else {
        options["luminosity-function-schechter-l0-" + name].value() =
            boost::any(function.l);
      }
      options["luminosity-function-schechter-phi0-" + name].value() =
          boost::any(function.phi);
    }
 ...
 void  LuminosityPriorConfig::writeFolder(std::string folder, std::map<std::string, LuminosityPriorConfig> configs){
   ...
   for(auto& prior_pair : configs){
     QString file_name = QString::fromStdString(folder)+QDir::separator()+QString::fromStdString(prior_pair.first+".xml");
