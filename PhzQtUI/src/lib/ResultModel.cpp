...
void ResultModel::load() {
  ...
  QDirIterator directories(QString::fromStdString(root_path), QDir::Dirs | QDir::NoSymLinks | QDir::NoDotAndDotDot,
                           QDirIterator::NoIteratorFlags);
  while (directories.hasNext()) {
    directories.next();
    int number = 0;

    QDirIterator run_dir(directories.filePath(), QDir::Dirs | QDir::NoSymLinks | QDir::NoDotAndDotDot,
                         QDirIterator::NoIteratorFlags);
    ...
    if (number > 0) {
      cat_vector.push_back(std::make_tuple<std::string, int, std::string>(
          directories.fileName().toStdString(), int(number), directories.filePath().toStdString()));
    }
  }
