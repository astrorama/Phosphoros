...
void FormAuxDataManagement::setRepositories(DatasetRepo filter_repository, DatasetRepo seds_repository,
...
void FormAuxDataManagement::displayFilter() {
  ...
  for (int i = 0; i < treeModel_filter->rowCount(); i++) {
    addDeleteButtonsToItem(treeModel_filter->item(i), treeModel_filter, ui->treeView_ManageFilter,
                           SLOT(deletFilterGroupButtonClicked(const QString&)), m_filter_del_buttons, 1);
  }
}
...
void FormAuxDataManagement::displaySED() {
  ...
  for (int i = 0; i < treeModel_Sed->rowCount(); i++) {
    ...
    addDeleteButtonsToItem(treeModel_Sed->item(i), treeModel_Sed, ui->treeView_ManageSed,
                           SLOT(deletSedGroupButtonClicked(const QString&)), m_sed_del_buttons, 2);
  }
...
void FormAuxDataManagement::displayRed() {
  ...
  for (int i = 0; i < treeModel_Red->rowCount(); i++) {
    addDeleteButtonsToItem(treeModel_Red->item(i), treeModel_Red, ui->treeView_ManageRed,
                           SLOT(deletRedGroupButtonClicked(const QString&)), m_red_del_buttons, 1);
  }
}
...
void FormAuxDataManagement::displayLum() {
  ...
  for (int i = 0; i < treeModel_Luminosity->rowCount(); i++) {
    addDeleteButtonsToItem(treeModel_Luminosity->item(i), treeModel_Luminosity, ui->treeView_ManageLuminosity,
                           SLOT(deletLumGroupButtonClicked(const QString&)), m_lum_del_buttons, 1);
  }
}
...
void FormAuxDataManagement::addDeleteButtonsToItem(QStandardItem* item, TreeModel* treeModel, QTreeView* view,
                                                   const char*                                  slot,
...
void FormAuxDataManagement::deletFilterGroupButtonClicked(const QString& group) {
  ...
  msgBox.setInformativeText(
      QString::fromStdString("You are about to delete the filter group ") + group +
      QString::fromStdString(
          ". \n\nThis is an irreversible action: the corresponding folder will be deleted from your file system.") +
      QString::fromStdString(" \n\nIf you where using some of these filters in any Catalog no further processing of it "
                             "will be possible!"));
...
void FormAuxDataManagement::deletSedGroupButtonClicked(const QString& group) {
  ...
  msgBox.setIcon(QMessageBox::Warning);
  msgBox.setInformativeText(
      QString::fromStdString("You are about to delete the SED group ") + group +
      QString::fromStdString(
          ". \n\nThis is an irreversible action: the corresponding folder will be deleted from your file system.") +
      QString::fromStdString(" \n\nIf you where using some of these SEDs in any Parameter Space no further processing "
...
void FormAuxDataManagement::deletRedGroupButtonClicked(const QString& group) {
  ...
  msgBox.setIcon(QMessageBox::Warning);
  msgBox.setInformativeText(
      QString::fromStdString("You are about to delete the Reddening Curve group ") + group +
      QString::fromStdString(
          ". \n\nThis is an irreversible action: the corresponding folder will be deleted from your file system.") +
      QString::fromStdString(" \n\nIf you where using some of these Reddening Curves in any Parameter Space no further "
                             "processing of it will be possible!"));
...
void FormAuxDataManagement::deletLumGroupButtonClicked(const QString& group) {
  ...
  msgBox.setIcon(QMessageBox::Warning);
  msgBox.setInformativeText(
      QString::fromStdString("You are about to delete the Luminosity Function Curve group ") + group +
      QString::fromStdString(
          ". \n\nThis is an irreversible action: the corresponding folder will be deleted from your file system.") +
      QString::fromStdString(" \n\nIf you where using some of these Luminosity Function Curve in any Luminosity Prior "
                             "configuration no further processing with it will be possible!"));
...
void FormAuxDataManagement::on_btn_import_filter_clicked() {
  ...
  if (dialog.exec()) {
    ...
    QObject::connect(worker, SIGNAL(finished(bool, QVector<QString>)),
                     SLOT(copyingFilterFinished(bool, QVector<QString>)));
...
void FormAuxDataManagement::on_btn_import_sed_clicked() {
  ...
  if (dialog.exec()) {
    ...
    auto worker = new FileCopyer(local);
    QObject::connect(worker, SIGNAL(finished(bool, QVector<QString>)),
                     SLOT(copyingSEDFinished(bool, QVector<QString>)));
    QObject::connect(worker, SIGNAL(copyProgress(qint64, qint64)), SLOT(copyProgress(qint64, qint64)));
    worker->setSourcePaths(input);
    worker->setDestinationPaths(output);
    local->start();
...
void FormAuxDataManagement::on_btn_import_reddening_clicked() {
  ...
  if (dialog.exec()) {
    ...
    auto worker = new FileCopyer(local);
    QObject::connect(worker, SIGNAL(finished(bool, QVector<QString>)),
                     SLOT(copyingRedFinished(bool, QVector<QString>)));
    QObject::connect(worker, SIGNAL(copyProgress(qint64, qint64)), SLOT(copyProgress(qint64, qint64)));
    worker->setSourcePaths(input);
    worker->setDestinationPaths(output);
    local->start();
...
void FormAuxDataManagement::on_btn_import_luminosity_clicked() {
  ...
  if (dialog.exec()) {
    ...
    auto worker = new FileCopyer(local);
    QObject::connect(worker, SIGNAL(finished(bool, QVector<QString>)),
                     SLOT(copyingLumFinished(bool, QVector<QString>)));
    QObject::connect(worker, SIGNAL(copyProgress(qint64, qint64)), SLOT(copyProgress(qint64, qint64)));
    worker->setSourcePaths(input);
    worker->setDestinationPaths(output);
    local->start();
...
void FormAuxDataManagement::on_btn_planck_clicked() {
  ...
  QString end_message = file_exists
                            ? "The file is already present: clicking OK will owerride it"
                            : "This is needed before using the Milky Way absorption correction option that you choose.";

  if (QMessageBox::Ok ==
      QMessageBox::question(this, "Dust map file Download...",
    ...
    connect(m_reply, SIGNAL(finished()), this, SLOT(httpReadyPlanckRead()));
