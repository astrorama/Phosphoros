...
void DataPackHandler::getDPVersionFinished(int, QProcess::ExitStatus){
  ...
  if (f.good()) {
    ...
    if ((remote != local || m_force) && QMessageBox::Yes == QMessageBox::question(m_parent, "New Data Package Avalable ...",
       ...
       connect(get_conflict_process, SIGNAL(finished(int, QProcess::ExitStatus)),
           this, SLOT(getConflictFinished(int, QProcess::ExitStatus)));
        ...
        /*dbg   << QString::fromStdString("--repo-url") << QString::fromStdString("http://localhost:8001") */
            ...
            << QString::fromStdString("--output-conflict") <<  QString::fromStdString(m_conflict_file);
...
void DataPackHandler::getConflictFinished(int, QProcess::ExitStatus) {
  ...
  if (info.exists()) {
    ...
    if (popUp->exec() == QDialog::Accepted) {
      ...
      connect(get_resolution_process, SIGNAL(finished(int, QProcess::ExitStatus)),
          this, SLOT(getResolutionFinished(int, QProcess::ExitStatus)));
