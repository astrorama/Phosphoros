...
DialogLuminosityFunction::DialogLuminosityFunction(DatasetRepo luminosity_repository, QWidget* parent)
    : QDialog(parent), ui(new Ui::DialogLuminosityFunction) {
...
void DialogLuminosityFunction::setInfo(LuminosityFunctionInfo info, size_t x, size_t y) {
  ...
  ui->lbl_redshift->setText(QString::number(m_FunctionInfo.z_min, 'f', 2) + " - " +
                            QString::number(m_FunctionInfo.z_max, 'f', 2));
...
void DialogLuminosityFunction::on_btn_tophat_clicked() {
  ...
  // call the script
  std::string command = "CreateSquareLuminosityCurve --step-begin " + std::to_string(m_min) + " --step-stop " +
                        std::to_string(m_max) + " --output-file \"" + path + "/" + name + ".txt" + "\"";
...
void DialogLuminosityFunction::on_btn_curve_clicked() {
  std::unique_ptr<DialogLuminosityFunctionCurveSelector> dialog(
      new DialogLuminosityFunctionCurveSelector(m_luminosity_repository));
